...
从上面的`spring-boot-starter-dependencies`的pom.xml中我们可以发现，一部分坐标的版本、依赖管理、插件管理已经定义好，所以创建的SpringBoot工程继承`spring-boot-starter-parent`后已经具备版本锁定等配置了。**所以起步依赖的作用就是进行依赖的传递。**
