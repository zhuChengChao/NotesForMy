...
注意：还有一个适配器模式是**接口适配器模式**。当不希望实现一个接口中所有的方法时【SDCard】，可以创建一个抽象类Adapter ，抽象类中去实现所有方法。而此时我们只需要继承该抽象类即可。
...
Reader（字符流，字符输入流顶层父类）、InputStream（字节流，字节输入流顶层父类）的适配使用的是 InputStreamReader【将字节数据转换成字符数据】。

InputStreamReader继承自`java.io`包中的Reader，对Reader中的抽象的未实现的方法给出实现。如：

```java
public class InputStreamReader extends Reader {
		
    // public class StreamDecoder extends Reader，聚合了类StreamDecoder
...
如上代码中的sd（StreamDecoder类对象【Decoder=解码，将字节数据转换为字符数据】【encoder=编码，将字符数据转换为字节数据】【适配器类，继承了抽象类Reader】），在Sun的JDK实现中，实际的方法实现是对`sun.nio.cs.StreamDecoder`类的同名方法的调用封装。类结构图如下：

![](image/适配器模式-jdk源码解析.png)
...
<font color="red">结论：</font>从表层来看，InputStreamReader做了InputStream字节流类到Reader字符流之间的转换。而从如上Sun JDK中的实现类关系结构中可以看出，是StreamDecoder的设计实现在实际上采用了对象适配器模式。
...
<img src="image/装饰者模式-使用前.png" style="zoom:80%;" />
...
**装饰者模式定义：**指在不改变现有对象结构的情况下，动态地给该对象增加一些职责（即增加其额外功能）的模式。
...
<img src="image/装饰者模式.png" style="zoom:75%;" />

代码如下：

```java
// 【抽象构件（Component）角色】快餐接口
@Data
...
// 【具体装饰（ConcreteDecorator）角色】鸡蛋配料
...
// 【具体装饰（ConcreteDecorator）角色】培根配料
...
public class Client {
    public static void main(String[] args) {
        ...
        System.out.println("========");
        // 点一份加鸡蛋的炒饭
        FastFood friedRiceWithEgg = new FriedRice();
        egg = new Egg(friedRiceWithEgg);
        //花费的价格
...
* 饰者模式可以带来比继承更加灵活性的扩展功能，使用更加方便，可以通过**组合不同的装饰者对象**来获取具有不同行为状态的多样化的结果。装饰者模式比继承更具良好的扩展性，完美的遵循开闭原则，继承是静态的附加责任，装饰者则是动态的附加责任。
...
<img src="image/装饰者模式-jdk源码.png" style="zoom:80%;" />

> <font color="red">小结：</font>`BufferedWriter`使用装饰者模式对`Writer`子实现类进行了增强，添加了缓冲区，提高了写数据的效率。
...
![桥接模式引入](image/桥接模式引入.png)

我们可以发现有很多的类，假如我们再增加一个形状或再增加一种颜色，就需要创建更多的类。

试想，在一个有多种可能会变化的维度的系统中，用继承方式会造成类爆炸，扩展起来不灵活。每次在一个维度上新增一个具体实现都要增加多个子类。为了更加灵活的设计系统，我们此时可以考虑使用桥接模式。

**定义：**将抽象与实现分离，使它们可以独立变化。它是**用组合关系代替继承关系来实现**，从而降低了抽象和实现这两个可变维度的耦合度。
...
需要开发一个跨平台视频播放器，可以在不同操作系统平台（如Windows、Mac、Linux等）上播放多种格式的视频文件，常见的视频格式包括RMVB、AVI、WMV等。该播放器包含了两个维度（操作系统/视频格式），适合使用桥接模式。

类图如下：

<img src="image/桥接模式.png" style="zoom:80%;" />

代码如下：

```java
// 视频文件【实现化（Implementor）角色】
public interface VideoFile {
    void decode(String fileName);
}

// avi文件【具体实现化（Concrete Implementor）角色】
...
// rmvb文件【具体实现化（Concrete Implementor）角色】
...
// Windows版本【扩展抽象化（Refined  Abstraction）角色 】
...
// mac版本【扩展抽象化（Refined  Abstraction）角色 】
...
<img src="image/外观模式.png" style="zoom:80%;" />

代码如下：

```java
// 灯类【子系统（Sub System）角色】
public class Light {
    public void on() {
...
![ServletRequest](image/ServletRequest.png)

RequestFacade类就使用了外观模式。先看结构图：

<img src="image/外观模式-jdk源码解析.png" style="zoom:70%;" />
...
<img src="image/文件系统.png" alt="文件系统" style="zoom:50%;" />
...
<img src="image/菜单案例.png" alt="菜单案例" style="zoom:60%;" />

要实现该案例，我们先画出类图：

<img src="image/组合模式.png" style="zoom:80%;" />
...
```java
// 【抽象根节点（Component）】菜单组件-不管是菜单还是菜单项，都应该继承该类
...
这里的MenuComponent定义为抽象类，因为有一些共有的属性和行为要在该类中实现，Menu和MenuItem类就可以**只覆盖自己感兴趣的方法**，而不用搭理不需要或者不感兴趣的方法，举例来说，Menu类可以包含子菜单，因此需要覆盖`add()`、`remove()`、`getChild()`方法，但是MenuItem就不应该有这些方法。这里给出的默认实现是抛出异常，你也可以根据自己的需要改写默认实现。
...
* 透明组合模式
  ...
  透明组合模式中，抽象根节点角色中声明了所有用于管理成员对象的方法，比如在示例中 `MenuComponent` 声明了 `add`、`remove` 、`getChild` 方法，这样做的好处是确保所有的构件类都有相同的接口。透明组合模式也是**组合模式的标准形式**。
...
* 安全组合模式
  ...
  <img src="image/组合模式-安全性.png" style="zoom:80%;" />
...
<img src="image/俄罗斯方块.jpeg" style="zoom:60%;" />

**先来看类图：**

<img src="image/享元模式.png" style="zoom:80%;" />
...
<img src="image/Integer类.png" style="zoom:80%;" />
